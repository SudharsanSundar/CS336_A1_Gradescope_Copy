...
@profile
def update_tokenization_of_pretoken(pretoken_tokens, merge_pair, merged_token, pretoken_count, pair_counts) -> (List[bytes], Dict[Tuple[bytes, bytes], int]):
    new_pretoken_tokens = []
    i = 0
...
@profile
def redo_pair_counts_after_merge(pretoken_data, pretoken, merged_token, pair_counts, pairs_to_pretokens, pretoken_count) -> (Dict[Tuple[bytes, bytes], int], Dict[Tuple[bytes, bytes], List[str]]):
    for first_tok, second_tok in zip(pretoken_data[pretoken][1][:-1], pretoken_data[pretoken][1][1:]):
        pair = (first_tok, second_tok)
...
@profile
def update_pretoken_data_fast(pretoken_data, pairs_to_pretokens, merge_pair, pair_counts) -> (Dict[str, List], Dict[Tuple[bytes, bytes], int], Dict[Tuple[bytes, bytes], List[str]]):
    ...
    for pretoken in pairs_to_pretokens[merge_pair]:
        ...
        new_pretoken_tokens, pair_counts = update_tokenization_of_pretoken(pretoken_tokens=pretoken_tokens, merge_pair=merge_pair, merged_token=merged_token, pretoken_count=pretoken_count, pair_counts=pair_counts)
        pretoken_data[pretoken][1] = new_pretoken_tokens

        # Redo pair counts based on new tokenization
        pair_counts, pairs_to_pretokens = redo_pair_counts_after_merge(pretoken_data=pretoken_data, pretoken=pretoken, merged_token=merged_token, pair_counts=pair_counts, pairs_to_pretokens=pairs_to_pretokens, pretoken_count=pretoken_count)

    # ppr.pprint(pretoken_data)
    # ppr.pprint(pair_counts)
    # ppr.pprint(pairs_to_pretokens)

    return pretoken_data, pair_counts, pairs_to_pretokens
...
def train_tokenizer(input_path: str,
    ...
    while len(vocabulary) < vocab_size:
        ...
        pretoken_data, pair_counts, pairs_to_pretokens = update_pretoken_data_fast(pretoken_data=pretoken_data, pairs_to_pretokens=pairs_to_pretokens, merge_pair=merges[-1], pair_counts=pair_counts)

    overall_end = time.time()

    with open('trained_vocab_ts.json', 'w') as f:
...
def main():
    ...
    if len(args) > 1:
        ...
        fp = '../../../data/TinyStoriesV2-GPT4-train.txt'
        vocab_size = 10000
