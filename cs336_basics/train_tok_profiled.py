...
def merge_step(pair_counts, vocabulary, idx, merges):
    ...
    return idx, merged_token


# @profile
def map_pairs_to_pretokens(pretoken_data) -> (Dict[Tuple[bytes, bytes], int], Dict[Tuple[bytes, bytes], Set[str]]):
    pair_counts = {}
    pairs_to_pretokens = {}
...
# @profile
def redo_pair_counts_after_merge(pretoken_data, pretoken, merged_token, pair_counts, pairs_to_pretokens,
                                 pretoken_count):
    for first_tok, second_tok in zip(pretoken_data[pretoken][1][:-1], pretoken_data[pretoken][1][1:]):
        pair = (first_tok, second_tok)
...
def update_pretoken_data_fast(pretoken_data, pairs_to_pretokens, merge_pair, pair_counts):
    ...
    for pretoken in pairs_to_pretokens[merge_pair]:
        ...
        new_pretoken_tokens = update_tokenization_of_pretoken(pretoken_tokens=pretoken_tokens, merge_pair=merge_pair,
                                                              merged_token=merged_token, pretoken_count=pretoken_count,
                                                              pair_counts=pair_counts)
        pretoken_data[pretoken][1] = new_pretoken_tokens

        # Redo pair counts based on new tokenization
        redo_pair_counts_after_merge(pretoken_data=pretoken_data, pretoken=pretoken, merged_token=merged_token,
                                     pair_counts=pair_counts, pairs_to_pretokens=pairs_to_pretokens,
...
def merges_formatting(merges):
    ...
    for elem in merges:
        new_merges.append(
            [elem[0].decode(encoding=ENCODING, errors='replace'), elem[1].decode(encoding=ENCODING, errors='replace')])
...
def train_tokenizer(input_path: str,
    ...
    '''
        TODO:
        (5) run owt for a limited time while profiled, and figure out what's taking all that time
        ---
        (4) [merge], scales with vocab size so might be why OWT is tough
        don't know how to make this faster yet
        (1) replace pretokenize findall with finditer -> not even needed for default
        maybe findall is too intensive, and I should use finditer instead. easy replace, since only used as iter
        (3) [make digestion faster?] 
        maybe double indexing is taking time when digesting? [how else could digestion be optimized]
        (2) load file chunk by chunk, instead of all at once -> needs to happen across all data to get right frequencies
        maybe reading in all the text at once is slowing things down as well. definitely taking lots of memory. Maybe I should stream it?
    ...
    while len(vocabulary) < vocab_size:
        ...
        idx, merged_token = merge_step(pair_counts=pair_counts, vocabulary=vocabulary, merges=merges, idx=idx)

        # Update pretoken data and counts
        update_pretoken_data_fast(pretoken_data=pretoken_data, pairs_to_pretokens=pairs_to_pretokens,
                                  merge_pair=merges[-1], pair_counts=pair_counts)
