...
def sample_ten_docs_owt(fp):
    ...
    return ''.join(samples.split('<|endoftext|>')[:10])


def main():
    ts_tokker = Tokenizer()
    ts_tokker = ts_tokker.from_files(vocab_filepath='../trained_vocab_ts.json', merges_filepath='../merges_ts.json',
                                     special_tokens=['<|endoftext|>'])

    ids = []
    with open('../tests/fixtures/tinystories_sample_5M.txt', 'r') as f:
        for id in ts_tokker.encode_iterable(f):
            ids += [id]
            if len(ids) % 1000 == 0:
                print(len(ids))

    # ts_tokker = Tokenizer()
    # ts_tokker = ts_tokker.from_files(vocab_filepath='./trained_vocab_ts.json', merges_filepath='./merges_ts.json', special_tokens=['<|endoftext|>'])
    #
    # ts = '../../../data/TinyStoriesV2-GPT4-train.txt'
    # owt = '../../../data/owt_train_2G.txt'
